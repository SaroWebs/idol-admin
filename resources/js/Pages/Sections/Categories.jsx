...
const Categories = (props) => {
  ...
  const normalizedData = categories.map((category) => ({
    ...
    icon: category.icon_path ? (
      <img src={`/storage/${category.icon_path}`} alt={category.name} width="40" height="40" />
    ) : (
      <img src={`/assets/images/no-image.png`} alt={category.name} width="40" height="40" />
    ),
    image: category.image_path ? (
      <img src={`/storage/${category.image_path}`} alt={category.name} width="60" height="60" />
    ) : (
      <img src={`/assets/images/no-image.png`} alt={category.name} width="40" height="40" />
    ),
  ...
  return (
    <MasterLayout {...props}>
      ...
      <div className="p-6">
        ...
        />
        {addFormOpen ? (
          <AddCategory reload={getCategories} setAddFormOpen={setAddFormOpen} />
        ) : (
          <div className="content">
            <div className="headings flex justify-between">
              <h1 className="text-2xl font-bold my-4">Categories</h1>
              <div className="actions flex gap-1">
                <Button color={'orange'} onClick={() => setAddFormOpen(true)}>
                  New Category
                </Button>
              </div>
            </div>
            {categories && <MantineReactTable columns={columns} data={normalizedData} />}
          </div>
        )}
      </div>
    </MasterLayout>
